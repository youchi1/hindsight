...
class TestMemoryEngineApiKeyGate:
    """MemoryEngine.__init__ checks for an LLM API key before constructing
    LLMConfig. That check must defer to the OpenClaw resolver when
    LLM_AUTH_SOURCE=openclaw, otherwise the daemon crashes on startup for
    key-required providers (anthropic, openai, gemini, …) even though
    auth-profiles.json holds the secret.
    """
    ...
    @staticmethod
    def _stub_engine_internals(monkeypatch: pytest.MonkeyPatch) -> None:
        """Stub everything MemoryEngine.__init__ does after the gate, so the
        test can construct an engine without a real DB/embeddings/LLM."""
        from hindsight_api.engine import memory_engine as me_module
        from hindsight_api.engine import llm_wrapper as llm_module
        ...
        monkeypatch.setattr(me_module, "create_embeddings_from_env", lambda: None)

        class _StubBackend:
            def create_task_backend(self, pool_getter, schema_getter):
                return None

            def get_pool(self):
                return None

        monkeypatch.setattr(me_module, "create_database_backend", lambda _t: _StubBackend())
        monkeypatch.setattr(me_module, "create_sql_dialect", lambda _b: None)

    def test_gate_passes_with_openclaw_auth_and_empty_key(self, auth_dir, monkeypatch):
        """Reproduces the production crash: anthropic + empty LLM_API_KEY +
        auth_source=openclaw should not raise the api-key ValueError."""
        from hindsight_api.engine.memory_engine import MemoryEngine

        self._set_openclaw_env(monkeypatch, auth_dir)
        self._stub_engine_internals(monkeypatch)

        # Should not raise. Pre-fix this raised
        # "LLM API key is required. Set HINDSIGHT_API_LLM_API_KEY environment variable."
        MemoryEngine(
    ...
    def test_gate_still_raises_without_auth_source(self, monkeypatch):
        """Regression guard: when auth_source is not openclaw, empty key on a
        key-required provider must still raise — we haven't loosened the
        check, only added one allowed bypass."""
        from hindsight_api.engine.memory_engine import MemoryEngine
